...
(function ($) {
    ...
    //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    $.extend($.fn.datagrid.methods, {
        /** wangmx 2014-11-11 扩展easyUI-tooltip
         * 开打提示功能    
         * @param {} jq    
...
(function ($) {
    ...
    $.extend($.fn.treegrid.methods, {
        ...
        cascadeCheck: function (target, param) {
            ...
            if ($(target).treegrid('getParent', param.id) != null)
                childrenNodes = $(target).treegrid('getParent', param.id).children;
            var selectChildrenNodesCount = 0;
            var selectChildrenNodesFlag = true;
            if (childrenNodes != null) {
                for (var i = 0; i < childrenNodes.length; i++) {
                    for (var j = 0; j < selectNodes.length; j++) {
                        if (selectNodes[j].TreeID == childrenNodes[i].TreeID) {
                            selectChildrenNodesCount++;
                            break;
                        }
                    }
                }
                if (selectChildrenNodesCount != childrenNodes.length)
                    selectChildrenNodesFlag = false;
            }
            //console.info(selectChildrenNodesCount);
            for (var i = 0; i < selectNodes.length; i++) {
                if (selectNodes[i][idField] == param.id)
                    status = true;
            }
            if (!status)
                selectChildrenNodesFlag = true;
            //级联选择父节点
            if (selectChildrenNodesFlag) {
